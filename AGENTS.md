...
Node is pinned via `.nvmrc` to `24.14.0`. Vitest covers `src/utils/semver.ts`, `src/utils/date.ts`, `src/api/queries.ts`, and `scripts/fetch-metadata.ts`'s `detectEsmSupport` (4 test files, 32 tests). No Storybook, no commitlint/semantic-release in this app.
...
- **Pending redesign**: ADR-0004 (three-axis verdicts) and ADR-0005 (single runtime engine) are accepted but not implemented. The code below still has two engines, a binary `compatible` flag and an ESM axis. Before touching compatibility logic, read `docs/adr/0004-three-axis-verdict-model.md`, `docs/adr/0005-single-runtime-engine.md` and `CONTEXT.md`. Move toward them; do not extend the two-engine design.
