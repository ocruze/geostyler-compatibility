...
import { detectModuleSystem, processNpmData, resolveTransitiveCoreRanges } from './fetch-metadata';
...
describe('detectModuleSystem', () => {
  ...
  it('esm when exports has an import condition', () => {
    expect(detectModuleSystem({ exports: { '.': { import: './x.js' } } })).toBe('esm');
  });
  it('esm when a module field is present', () => {
    expect(detectModuleSystem({ module: './x.mjs' })).toBe('esm');
  });
