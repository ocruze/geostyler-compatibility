...
Node is pinned via `.nvmrc` to `24.14.0`. Vitest covers `src/utils/semver.ts`, `src/api/queries.ts`, and `scripts/fetch-metadata.ts`'s `detectEsmSupport` (3 test files, 29 tests). No Storybook, no commitlint/semantic-release in this app.
