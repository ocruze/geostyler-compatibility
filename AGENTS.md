...
Node is pinned via `.nvmrc` to `24.14.0`. Vitest covers `src/utils/semver.ts`, `src/utils/date.ts`, `src/api/queries.ts`, and `scripts/fetch-metadata.ts` (`processNpmData`, `detectModuleSystem`, `detectEsmSupport`) against recorded registry responses in `scripts/__fixtures__/registry/` (4 test files, 45 tests). No Storybook, no commitlint/semantic-release in this app.
...
- **Pending redesign**: ADR-0004 (three-axis verdicts) and ADR-0005 (single runtime engine) are accepted but not implemented. The code below still has two engines, a binary `compatible` flag and an ESM axis. The dataset already carries the three-axis inputs (#30, expand half): read `coreRanges`/`declaredDependencies`/`moduleSystem` in new code, not `geostylerStyleRange`/`esmSupport`. Before touching compatibility logic, read `docs/adr/0004-three-axis-verdict-model.md`, `docs/adr/0005-single-runtime-engine.md` and `CONTEXT.md`. Move toward them; do not extend the two-engine design.
...
- **Routing**: file-based routes in `src/routes/` (`__root.tsx`, `index.tsx`, `overview.tsx`, `compare.tsx`, `docs.tsx`, `package.$name.tsx`). `index.tsx` only redirects `/` to `/overview`; the landing page is `overview.tsx`. `src/routeTree.gen.ts` is **generated** by `@tanstack/router-plugin` (Vite) — do not hand-edit.
...
- **Data pipeline** (build-time, Node via `tsx`): `scripts/fetch-metadata.ts` reads npm registry metadata for the tracked packages and emits `src/data/packages.json` (per-version `coreRanges` keyed by core package with a `source` of `declared` or `none`, `declaredDependencies` on tracked packages only, `peerDependencies`, `moduleSystem` of `esm`/`cjs`/`types-only`, plus the legacy `geostylerStyleRange`/`esmSupport` the old engine still reads, and Package-level `format`), tracking per-package fetch failures and setting `process.exitCode = 1` if any occur; `scripts/compute-compatibility.ts` computes the pairwise (+ some UI/parser triplet) compatibility matrix into `src/data/compatibility-matrix.json`. The `packages.json` file is `{ generatedAt, packages }`. The footer in `src/routes/__root.tsx` shows `generatedAt` as a UTC date.
